...
LRGASP_calculations_challenge3 <- function (NAME, class.file, junc.file, out.dir, platform, functions.dir, bam, organism, challenges_ids) {
...
### remove SIRV and ERCC transcripts from sqanti data
  ...
  for (challenge_name in challenges_ids) {
    if (challenge_name == "mouse_sirvs") {
      precision_recall_results("Precision", as.numeric(b.SIRVs_results["Precision","Value"]), challenge_name)
      precision_recall_results("Sensitivity", as.numeric(b.SIRVs_results["Sensitivity","Value"]), challenge_name)
    }
