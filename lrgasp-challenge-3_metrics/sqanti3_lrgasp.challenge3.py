...
class myQueryTranscripts:
    ...
    def __str__(self):
        return "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (
...
def correctionPlusORFpred(args, genome_dict):
    ...
    if args.skipORF:
        ...
        for r in SeqIO.parse(open(corrORF), 'fasta'):
            ...
            if m is None:
                print(
                    "Expected GMST output IDs to be of format '<pbid> gene_4|GeneMark.hmm|<orf>_aa|<strand>|<cds_start>|<cds_end>' but instead saw: {0}! Abort!".format(
...
def reference_parser(args, genome_chroms):
    ...
    return dict(refs_1exon_by_chr), dict(refs_exons_by_chr), dict(junctions_by_chr), dict(junctions_by_gene), dict(
        known_5_3_by_gene)
...
def STARcov_parser(coverageFiles):  # just valid with unstrand-specific RNA-seq protocols.
    ...
    for file in cov_files:
        ...
        for r in STARJunctionReader(file):
            ...
            all_read += 1
    print("{0} junctions read. {1} junctions added to both strands because no strand information from STAR.".format(
        all_read, undefined_strand_count), file=sys.stderr)
...
def transcriptsKnownSpliceSites(refs_1exon_by_chr, refs_exons_by_chr, start_ends_by_gene, trec, genome_dict, nPolyA):
    ...
    if trec.exonCount >= 2:
        ...
        for ref_gene in hits_by_gene:
            ...
            for ref in hits_by_gene[ref_gene]:
                ...
                if ref.exonCount == 1:  # mono-exonic reference, handle specially here
                    if calc_exon_overlap(trec.exons, ref.exons) > 0 and cat_ranking[isoform_hit.str_class] < \
                    ...
                    match_type = compare_junctions(trec, ref, internal_fuzzy_max_dist=0, max_5_diff=999999,
                                                   max_3_diff=999999)
                    ...
                    if match_type == "exact":
                        ...
                        if cat_ranking[isoform_hit.str_class] < cat_ranking["full-splice_match"] or \
                            ...
                            isoform_hit = myQueryTranscripts(trec.id, diff_tss, diff_tts, trec.exonCount, trec.length,
                                                             ...
                                                             refStart=ref.txStart,
                                                             refEnd=ref.txEnd,
                                                             q_splicesite_hit=calc_splicesite_agreement(trec.exons,
                                                                                                        ref.exons),
                                                             q_exon_overlap=calc_exon_overlap(trec.exons, ref.exons),
                                                             percAdownTTS=str(percA),
                                                             seqAdownTTS=seq_downTTS)
                    ...
                    # #######################################################
                        ...
                        if cat_ranking[isoform_hit.str_class] < cat_ranking["incomplete-splice_match"] or \
                                (isoform_hit.str_class == 'incomplete-splice_match' and abs(diff_tss) + abs(
                                    diff_tts) < isoform_hit.get_total_diff()):
                            isoform_hit = myQueryTranscripts(trec.id, diff_tss, diff_tts, trec.exonCount, trec.length,
                    ...
                    # #######################################################
                        ...
                        if cat_ranking[isoform_hit.str_class] < cat_ranking["anyKnownJunction"] or \
                                ...
                                (
                                        isoform_hit.str_class == 'anyKnownJunction' and q_sp_hit == isoform_hit.q_splicesite_hit and q_exon_d < abs(
                                    trec.exonCount - isoform_hit.refExons)):
                            isoform_hit = myQueryTranscripts(trec.id, "NA", "NA", trec.exonCount, trec.length,
                        ...
                        if cat_ranking[isoform_hit.str_class] < cat_ranking[
                            "anyKnownSpliceSite"] and calc_splicesite_agreement(trec.exons, ref.exons) > 0:
                            isoform_hit = myQueryTranscripts(trec.id, "NA", "NA", trec.exonCount, trec.length,
                        ...
                        if isoform_hit.str_class == "":  # still not hit yet, check exonic overlap
                            if cat_ranking[isoform_hit.str_class] < cat_ranking["geneOverlap"] and calc_exon_overlap(
                                    trec.exons, ref.exons) > 0:
                                isoform_hit = myQueryTranscripts(trec.id, "NA", "NA", trec.exonCount, trec.length,
        ...
        best_by_gene = [
            geneHitTuple(cat_ranking[iso_hit.str_class], iso_hit.refStart, iso_hit.refEnd, ref_gene, iso_hit) for
            ref_gene, iso_hit in best_by_gene.items()]
        ...
        best_by_gene.sort(key=lambda x: (x.score, x.iso_hit.q_splicesite_hit + (x.iso_hit.q_exon_overlap) * 1. / sum(
            e.end - e.start for e in trec.exons) + calc_overlap(x.rStart, x.rEnd, trec.txStart, trec.txEnd) * 1. / (
                                                     x.rEnd - x.rStart) - abs(trec.exonCount - x.iso_hit.refExons)),
                          reverse=True)  # sort by (ranking score, overlap)
...
def novelIsoformsKnownGenes(isoforms_hit, trec, junctions_by_chr, junctions_by_gene, start_ends_by_gene):
    ...
    def has_intron_retention():
        for e in trec.exons:
            ...
            if m < len(junctions_by_chr[trec.chrom]['da_pairs']) and e.start <= \
                    junctions_by_chr[trec.chrom]['da_pairs'][m][0] < junctions_by_chr[trec.chrom]['da_pairs'][m][
                1] < e.end:
                return True
        return False
    ...
    if len(ref_genes) == 1:
        ...
        for d, a in trec.junctions:
            all_junctions_known = all_junctions_known and (d in junctions_by_chr[trec.chrom]['donors']) and (
                        a in junctions_by_chr[trec.chrom]['acceptors'])
        ...
        all_ref_junctions = list(
            itertools.chain(junctions_by_gene[ref_gene] for ref_gene in ref_genes if ref_gene in junctions_by_gene))
...
def write_junctionInfo(trec, junctions_by_chr, accepted_canonical_sites, indelInfo, genome_dict, fout, covInf=None,
                       covNames=None, phyloP_reader=None):
    """
    ...
    for junction_index, (d, a) in enumerate(trec.junctions):
        ...
        if phyloP_reader is not None:
            phyloP_start = ",".join([str(x) for x in
                                     [phyloP_reader.get_pos(trec.chrom, d - 1), phyloP_reader.get_pos(trec.chrom, d),
                                      phyloP_reader.get_pos(trec.chrom, d + 1)]])
            phyloP_end = ",".join(
                [str(x) for x in [phyloP_reader.get_pos(trec.chrom, a - 1), phyloP_reader.get_pos(trec.chrom, a),
        ...
        qj = {'isoform': trec.id,
              ...
              "diff_to_Ref_start_site": min_diff_s if min_diff_s == min_diff_s else "NA",
              # check if min_diff is actually nan,
              "diff_to_Ref_end_site": min_diff_e if min_diff_e == min_diff_e else "NA",
              # check if min_diff is actually nan,
              "bite_junction": "TRUE" if (
                          (min_diff_s < 0 or min_diff_e < 0) and not (min_diff_s > 0 or min_diff_e > 0)) else "FALSE",
...
def isoformClassification(args, isoforms_by_chr, refs_1exon_by_chr, refs_exons_by_chr, junctions_by_chr,
    ...
    for chrom, records in isoforms_by_chr.items():
        for rec in records:
            ...
            isoform_hit = transcriptsKnownSpliceSites(refs_1exon_by_chr, refs_exons_by_chr, start_ends_by_gene, rec,
                                                      genome_dict, nPolyA=args.window)
            ...
            write_junctionInfo(rec, junctions_by_chr, accepted_canonical_sites, indelsJunc, genome_dict, fout_junc,
                               covInf=SJcovInfo, covNames=SJcovNames, phyloP_reader=phyloP_reader)
            ...
            if polyA_motif_list is not None:
                if rec.strand == '+':
                    polyA_motif, polyA_dist = find_polyA_motif(
                        str(genome_dict[rec.chrom][rec.txEnd - 50:rec.txEnd].seq), polyA_motif_list)
                else:
                    polyA_motif, polyA_dist = find_polyA_motif(
                        str(genome_dict[rec.chrom][rec.txStart:rec.txStart + 50].reverse_complement().seq),
                        polyA_motif_list)
...
def FLcount_parser(fl_count_filename):
    ...
    if type == 'SINGLE_SAMPLE':
        ...
        print("Expected pbid or superPBID as a column in count file {0}. Abort!".format(fl_count_filename),
              file=sys.stderr)
        sys.exit(-1)
    f.close()
...
def run(args):
    ...
    isoforms_info = isoformClassification(args, isoforms_by_chr, refs_1exon_by_chr, refs_exons_by_chr, junctions_by_chr,
                                          junctions_by_gene, start_ends_by_gene, genome_dict, indelsJunc, orfDict)
    ...
    if args.expression:
        ...
        for iso in isoforms_info:
            if iso not in exp_dict:
                exp_dict[iso] = 0
                print("WARNING: isoform {0} not found in expression matrix. Assigning TPM of 0.".format(iso),
                      file=sys.stderr)
    ...
    sj_covs_by_isoform = defaultdict(
        lambda: [])  # pbid --> list of total_cov for each junction so we can calculate SD later
    for r in reader:
    ...
    if not args.skip_report:
        ...
        rdata_out = os.path.join(os.path.abspath(args.dir), args.output + "_JSON_data")
...
def run_isoAnnotLite(correctedGTF, outClassFile, outJuncFile, outDir, outName, gff3_opt):
    if gff3_opt:
        ISOANNOT_CMD = "python " + ISOANNOT_PROG + " {g} {c} {j} -gff3 {t} -d {d} -o {o}".format(g=correctedGTF,
                                                                                                 c=outClassFile,
                                                                                                 j=outJuncFile,
                                                                                                 t=gff3_opt, d=outDir,
                                                                                                 o=outName)
    else:
        ISOANNOT_CMD = "python " + ISOANNOT_PROG + " {g} {c} {j} -d {d} -o {o}".format(g=correctedGTF, c=outClassFile,
                                                                                       j=outJuncFile, d=outDir,
                                                                                       o=outName)
...
def combine_split_runs(args, split_dirs):
    ...
    if not args.skip_report:
        ...
        cmd = RSCRIPTPATH + " {d}/{f} {c} {j} {p} {d}".format(d=utilitiesPath, f=RSCRIPT_REPORT, c=outputClassPath,
                                                              j=outputJuncPath, p=args.doc)
        if subprocess.check_call(cmd, shell=True) != 0:
            print("ERROR running command: {0}".format(cmd), file=sys.stderr)
            sys.exit(-1)
...
def compute_metrics(args, split_dirs):
    ...
    for i, split_d in enumerate(split_dirs):
        _gtf, _sam, _fasta, _orf = get_corr_filenames(args, split_d)
        _class, _junc = get_class_junc_filenames(args, split_d)
        metrics.append(compute_metrics_single(args, _gtf, _class, _junc))
    return metrics
...
def main():
    ...
    parser.add_argument('ref_dir',
                        help= '\t Public reference directory including GTF annotation and genome')
    parser.add_argument("challenges_ids",
                        help="\t Space-separated list of challenge IDs to run on")
    ...
    parser.add_argument('-g', '--gtf', help='\t\tUse when running SQANTI by using as input a gtf of isoforms',
                        action='store_true')
    ...
    parser.add_argument('-c', '--coverage',
                        ...
                        required=False)
    parser.add_argument('-s', '--sites', default="ATAC,GCAG,GTAG",
    ...
    parser.add_argument("-v", "--version", help="Display program version number.", action='version',
                        version='SQANTI3 ' + str(__version__))
    parser.add_argument("--skip_report", action="store_true", default=False, help=argparse.SUPPRESS)
    parser.add_argument('--isoAnnotLite', help='\t\tRun isoAnnot Lite to output a tappAS-compatible gff3 file',
    ...
    if not args.challenges_ids:
        print("ERROR: Please provide a list of challenge IDs to run on")
        sys.exit(1)
    ...
    if args.is_fusion:
        if args.orf_input is None:
            print(
                "WARNING: Currently if --is_fusion is used, no ORFs will be predicted. Supply --orf_input if you want ORF to run!",
                file=sys.stderr)
            args.skipORF = True
        if not args.gtf:
    ...
    if args.gff3 is not None:
        ...
        if not os.path.isfile(args.gff3):
            print("ERROR: Precomputed tappAS GFF3 annoation file {0} doesn't exist. Abort!".format(args.genome),
                  file=sys.stderr)
            sys.exit(-1)
    ...
    print("**** SQANTI3 finished.", file=sys.stdout)
